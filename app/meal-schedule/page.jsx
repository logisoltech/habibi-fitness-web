...
export default function MealSchedule() {
  ...
  const handleSwapMeals = async (targetMeal) => {
    ...
    try {
      ...
      const sourceMeal = {
        ...
        dayKey: dayKey,
        mealKey: selectedMealToSwap.mealKey || selectedMealToSwap.category?.toLowerCase() || 'lunch',
  ...
  return (
    <>
      <Header />
      <div className={`${font.className} max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen mt-[100px] relative`}>
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-black mb-2">Meal Schedule</h1>
          <p className="text-gray-600">Plan and manage your meals for the month</p>
        </div>
            ...
            {mealsLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {meals.map((meal, index) => (
                  <div key={`${meal.id}-${index}`} className="bg-gray-50 rounded-2xl overflow-hidden hover:shadow-md transition-shadow duration-300">
                    {/* Meal Image */}
                    ...
                    {/* Meal Info */}
                    <div className="p-4">
                      <h3 className="font-bold text-lg text-gray-900 mb-2">{meal.title}</h3>
                      
                      {meal.subtitle && (
                        <p className="text-gray-600 text-sm mb-3 line-clamp-2">{meal.subtitle}</p>
                      )}
                      ...
                      {meal.category && (
                        <div className="flex items-center justify-between">
                          <span className="inline-block px-3 py-1 bg-green-100 text-green-700 text-sm rounded-full capitalize font-medium">
                            {meal.category}
                          </span>
                        </div>
                      )}
      ...
      {showSwapModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50">
          <div className="bg-white rounded-t-3xl w-full max-w-4xl h-[90vh] flex flex-col">
            {/* Modal Header */}
              ...
              {!selectedMealToSwap ? (
                ...
                <>
                  <p className="text-gray-600 mb-6 text-center">
                    Select which meal from {selectedDate.format('dddd, MMMM D')} you want to swap:
                  </p>
                  {meals && meals.length > 0 ? (
                    <div className="space-y-4">
                      {meals.map((meal, index) => (
                        <button
                          ...
                          className="w-full bg-gray-50 rounded-2xl overflow-hidden border-2 border-transparent hover:border-green-500 transition-colors"
                        >
                          <div className="flex">
                            ...
                            <div className="flex-1 p-4 text-left">
                              ...
                              <p className="text-gray-500 text-sm capitalize">{meal.category || 'Meal'}</p>
                            </div>
                          </div>
                        </button>
                      ))}
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500">No meals available for this day</p>
                    </div>
                  )}
                </>
                ...
                <>
                  <div className="flex items-center gap-3 bg-green-50 p-4 rounded-xl mb-6">
                    ...
                    <span className="text-gray-700">
                      Swapping: <span className="font-bold text-green-600">{selectedMealToSwap.title}</span>
                    </span>
                  </div>
                  
                  {(() => {
                    const selectedDayName = dayDisplayNames[activeDayTab];
                    ...
                    if (mealsForDay.length === 0) {
                      return (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-500">No meals available for {selectedDayName}</p>
                        </div>
                      );
                    }
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">No meals available for Week {activeWeekTab + 1}</p>
                </div>
              )}
            </div>
